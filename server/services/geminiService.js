...
const buildPrompt = (objectives, destinationName) => `
You are a travel cost estimation expert. City: ${destinationName}
...
export const estimateObjectiveCosts = async (objectives, destinationName) => {
  const prompt = buildPrompt(objectives, destinationName);
  ...
  for (const modelName of GEMINI_MODELS) {
    try {
      ...
      return parsed.map(item => ({
        ...
        estimated_cost: Math.max(0, Math.round(item.estimated_cost || 0))
      }));
...
// // ── Versiunea veche cu fallback nested (4 modele) ──────────────────────────
// export const estimateObjectiveCosts = async (objectives, destinationName) => {
//
//   const model = genAI.getGenerativeModel({
